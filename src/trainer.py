...
def evaluate_performance(model, test_dl):
    # Check if model is using MPS, CUDA, or CPU
    if torch.backends.mps.is_available():
        device = torch.device('mps')
        dtype = torch.float32  # MPS uses float32
    ...
    acc = 0
    for sample_batched in test_dl:
        x = sample_batched['features'].type(dtype).to(device)
        y = sample_batched['gt'].type(dtype).to(device)
...
def get_confusion_matrix(model_saved_state, dl=None):
    ...
    saved_state = torch.load('saved states/' + model_saved_state)
